...
REM ============================================================
REM 4-Group Ablation Study: Multi-Critic Baseline Comparison
REM Group A: Multi-Critic + Adaptive Lagrange (主线)
REM Group B: Multi-Critic + Fixed Lambda (惩罚法)
REM Group C: Multi-Critic + Lambda=0 (reward-only baseline)
REM Group D: Two-Critic + Aggregated Cost (新增 baseline)
REM ============================================================
...
%PY% %ENTRY% --output_dir %OUTDIR% --seed %SEED% --total_iters %ITERS% --batch_size %BATCH% --minibatch_size %MINIBATCH% --update_epochs %EPOCHS% --grid_size %GRID% --max_steps %MAX_STEPS% --success_reward %SUCCESS_R% --step_penalty %STEP_P% --energy_high_cost %E_HIGH_COST% --energy_high_density %E_HIGH_DENS% --congestion_pattern %CONG_PATTERN% --congestion_density %CONG_DENS% --load_cost_scale %LOAD_SCALE% --start_goal_mode %START_GOAL_MODE% --start_rect %START_RECT% --goal_rect %GOAL_RECT% --include_congestion_obs %INC_CONG_OBS% --congestion_patch_radius %CONG_R% --include_energy_obs %INC_E_OBS% --energy_patch_radius %E_R% --energy_obs_normalize %E_NORM% --obs_rms %OBS_RMS% --cost_critic_mode separate --use_lagrange True --lambda_gap_mode %GAP_MODE% --lambda_lr_energy %LAMBDA_LR_E% --lambda_lr_load %LAMBDA_LR_L% --lambda_update_freq %LAMBDA_UPDATE_FREQ% --lambda_max %LAMBDA_MAX% --dual_update_mode %DUAL_UPDATE_MODE% --energy_budget %E_BUDGET% --load_budget %L_BUDGET% --risk_factor %RISK_FACTOR% --initial_lambda_energy 0.0 --initial_lambda_load 0.0 --dual_gap_ema_beta %DUAL_GAP_BETA% --dual_corr_ema_beta %DUAL_CORR_BETA% --run_tag "%RUN_TAG%"
if errorlevel 1 goto :fail

REM ========== Group B: Multi-Critic + Fixed Lambda ==========
echo.
echo [GROUP B] Multi-Critic + Fixed Lambda (惩罚法)
echo ============================================================
set "RUN_TAG=B_multi_critic_fixedlambda_seed%SEED%"
%PY% %ENTRY% --output_dir %OUTDIR% --seed %SEED% --total_iters %ITERS% --batch_size %BATCH% --minibatch_size %MINIBATCH% --update_epochs %EPOCHS% --grid_size %GRID% --max_steps %MAX_STEPS% --success_reward %SUCCESS_R% --step_penalty %STEP_P% --energy_high_cost %E_HIGH_COST% --energy_high_density %E_HIGH_DENS% --congestion_pattern %CONG_PATTERN% --congestion_density %CONG_DENS% --load_cost_scale %LOAD_SCALE% --start_goal_mode %START_GOAL_MODE% --start_rect %START_RECT% --goal_rect %GOAL_RECT% --include_congestion_obs %INC_CONG_OBS% --congestion_patch_radius %CONG_R% --include_energy_obs %INC_E_OBS% --energy_patch_radius %E_R% --energy_obs_normalize %E_NORM% --obs_rms %OBS_RMS% --cost_critic_mode separate --use_lagrange False --lambda_gap_mode %GAP_MODE% --lambda_lr_energy %LAMBDA_LR_E% --lambda_lr_load %LAMBDA_LR_L% --lambda_update_freq %LAMBDA_UPDATE_FREQ% --lambda_max %LAMBDA_MAX% --dual_update_mode %DUAL_UPDATE_MODE% --energy_budget %E_BUDGET% --load_budget %L_BUDGET% --risk_factor %RISK_FACTOR% --initial_lambda_energy 0.5 --initial_lambda_load 0.5 --dual_gap_ema_beta %DUAL_GAP_BETA% --dual_corr_ema_beta %DUAL_CORR_BETA% --run_tag "%RUN_TAG%"
if errorlevel 1 goto :fail

REM ========== Group C: Multi-Critic + Lambda=0 ==========
echo.
echo [GROUP C] Multi-Critic + Lambda=0 (reward-only baseline)
echo ============================================================
set "RUN_TAG=C_multi_critic_l0_seed%SEED%"
%PY% %ENTRY% --output_dir %OUTDIR% --seed %SEED% --total_iters %ITERS% --batch_size %BATCH% --minibatch_size %MINIBATCH% --update_epochs %EPOCHS% --grid_size %GRID% --max_steps %MAX_STEPS% --success_reward %SUCCESS_R% --step_penalty %STEP_P% --energy_high_cost %E_HIGH_COST% --energy_high_density %E_HIGH_DENS% --congestion_pattern %CONG_PATTERN% --congestion_density %CONG_DENS% --load_cost_scale %LOAD_SCALE% --start_goal_mode %START_GOAL_MODE% --start_rect %START_RECT% --goal_rect %GOAL_RECT% --include_congestion_obs %INC_CONG_OBS% --congestion_patch_radius %CONG_R% --include_energy_obs %INC_E_OBS% --energy_patch_radius %E_R% --energy_obs_normalize %E_NORM% --obs_rms %OBS_RMS% --cost_critic_mode separate --use_lagrange False --lambda_gap_mode %GAP_MODE% --lambda_lr_energy %LAMBDA_LR_E% --lambda_lr_load %LAMBDA_LR_L% --lambda_update_freq %LAMBDA_UPDATE_FREQ% --lambda_max %LAMBDA_MAX% --dual_update_mode %DUAL_UPDATE_MODE% --energy_budget %E_BUDGET% --load_budget %L_BUDGET% --risk_factor %RISK_FACTOR% --initial_lambda_energy 0.0 --initial_lambda_load 0.0 --dual_gap_ema_beta %DUAL_GAP_BETA% --dual_corr_ema_beta %DUAL_CORR_BETA% --run_tag "%RUN_TAG%"
if errorlevel 1 goto :fail
...
echo.
echo ============================================================
echo DONE. All 4 groups completed successfully!
echo Results saved under: %OUTDIR%
echo   Group A (Adaptive):     %OUTDIR%\A_multi_critic_adaptive_seed%SEED%
echo   Group B (Fixed Lambda): %OUTDIR%\B_multi_critic_fixedlambda_seed%SEED%
echo   Group C (Lambda=0):     %OUTDIR%\C_multi_critic_l0_seed%SEED%
echo   Group D (Aggregated):   %OUTDIR%\D_two_critic_aggcost_adaptive_seed%SEED%
echo ============================================================
pause
exit /b 0
...
:fail
echo.
echo [ERROR] One of the group runs failed. Check the traceback above.
pause
exit /b 1
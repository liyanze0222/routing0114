...
def rollout_single_episode(
    ...
    # 注：不在这里打印警告，由compute_visitmap_stats汇总统计后报告
...
def compute_visitmap_stats(
    ...
    cell_of_interest2: Optional[Tuple[int, int]] = None,
) -> Dict:
    """
    统计多个 episode 的访问频率。
    
    Args:
        cell_of_interest2: 可选的第二个关注cell（用于双cell统计）
    ...
    Returns:
        {
            ...
            "hit_rate_cell": float,
            "hit_rate_cell2": float (if cell_of_interest2 is not None),
            "success_rate": float,
    ...
    total_len_success = 0
    no_goal_count = 0
    max_steps_failures = 0  # 统计走满max_steps的失败次数
    ...
    for ep in range(n_episodes):
        ...
        # 检查是否访问过 cell_of_interest2
        if cell_of_interest2 is not None and cell_of_interest2 in visited_cells:
            hit_count2 += 1
        ...
        # 统计走满max_steps的失败
        if result["episode_length"] >= max_steps_cfg and not result["success"]:
            max_steps_failures += 1
    ...
    # 汇总警告：如果有较多失败cases，打印一次性警告
    if max_steps_failures > 0:
        failure_rate = max_steps_failures / n_episodes
        if failure_rate > 0.1:  # 超过10%失败
            print(f"  ⚠️  {max_steps_failures}/{n_episodes} episodes hit max_steps without success ({failure_rate:.1%})")
            print(f"      This may indicate suboptimal policy quality.")
    ...
    if cell_of_interest2 is not None:
        hit_rate_cell2 = hit_count2 / max(1, n_episodes)
        result_dict["hit_rate_cell2"] = hit_rate_cell2
...
def find_max_change_cells(
    ...
    """
    找出访问概率变化最大的 cell（排除 start/goal）。
    
    Args:
        delta: visit_prob 差分矩阵 (grid_size, grid_size)
        start_rc: 起点坐标（若有）
        goal_rc: 终点坐标（若有）
        topk: 返回前几个
    
    Returns:
        [((r, c), delta_value), ...] 按 |delta| 降序排列
    """
    ...
    # 找出有效区域的最大值
    valid_values = abs_delta[mask]
    if len(valid_values) == 0 or np.max(valid_values) == 0:
        return []
    
    # 展平并排序
    flat_indices = np.argsort(abs_delta.ravel())[::-1]
    
    results = []
    for idx in flat_indices:
        r, c = np.unravel_index(idx, abs_delta.shape)
        if not mask[r, c]:
            continue
        if abs_delta[r, c] == 0:
            break
        results.append(((r, c), delta[r, c]))
        if len(results) >= topk:
            break
...
def plot_visitmap_diff(
    ...
    # 标注 cell_of_interest (cyan X)
    ax.scatter([cell_of_interest[1]], [cell_of_interest[0]], 
               marker="x", s=200, color="cyan", linewidths=3, label=f"Cell1 {cell_of_interest}")
    
    # 标注 cell_of_interest2 (white circle)
    if cell_of_interest2 is not None:
        ax.scatter([cell_of_interest2[1]], [cell_of_interest2[0]], 
                   marker="o", s=150, facecolors="none", edgecolors="white", linewidths=2.5,
                   label=f"Cell2 {cell_of_interest2}")
...
def run_visitmap_analysis(args, deterministic: bool, cell_of_interest: Tuple[int, int], cell_of_interest2_input: str):
    """
    运行访问频率分析模式（增强版：支持 cell2 自动选择）。
    
    Args:
        cell_of_interest2_input: "auto" or "r,c" format
    """
    # 查找可用 checkpoint
    ...
    print(f"\n{'='*70}")
    print(f"VISITMAP ANALYSIS MODE (ENHANCED)")
    ...
    print(f"Cell of Interest 2: {cell_of_interest2_input}")
    print(f"{'='*70}\n")
    
    # 创建输出目录
    out_dir = os.path.join(args.run_dir, "route_diag")
    ...
    # 为每个 checkpoint 统计访问频率（首次遍历：确定 cell2）
    all_stats = {}
    cell_of_interest2 = None
    
    # 第一次遍历：收集stats并自动检测cell2
    for ckpt_name, ckpt_path in checkpoints.items():
        ...
        stats = compute_visitmap_stats(
            ...
            args.device, max_steps_cfg, grid_size, cell_of_interest, None
        )
    ...
    # 自动检测 cell2
    if cell_of_interest2_input.lower() == "auto":
        if "best_tail.pt" in all_stats and "best_fsr.pt" in all_stats:
            print(f"\n--- Auto-detecting cell2 (max change cell) ---")
            tail_vp = all_stats["best_tail.pt"]["visit_prob"]
            fsr_vp = all_stats["best_fsr.pt"]["visit_prob"]
            delta = tail_vp - fsr_vp
            
            # 获取起终点信息
            env_temp = make_env_from_config(cfg, args.seed)
            obs, info = env_temp.reset(seed=args.seed)
            start_rc = info.get("start") or (env_temp.unwrapped.agent_row, env_temp.unwrapped.agent_col)
            goal_rc = info.get("goal") or (env_temp.unwrapped.goal_row, env_temp.unwrapped.goal_col)
            
            max_cells = find_max_change_cells(delta, start_rc, goal_rc, topk=args.topk_cells)
            if max_cells:
                cell_of_interest2, delta_val = max_cells[0]
                # 转换为标准Python int
                cell_of_interest2 = (int(cell_of_interest2[0]), int(cell_of_interest2[1]))
                print(f"   ✅ Auto-selected cell2: {cell_of_interest2} (delta={delta_val:+.4f})")
                
                # 打印 topk
                if len(max_cells) > 1 and args.topk_cells > 1:
                    print(f"   Top-{args.topk_cells} cells with max |delta|:")
                    for i, (cell, dv) in enumerate(max_cells, 1):
                        cell_clean = (int(cell[0]), int(cell[1]))
                        print(f"      #{i}: {cell_clean} -> delta={dv:+.4f}")
            else:
                print(f"   ⚠️  No significant change found, cell2=None")
        else:
            print(f"\n   ⚠️  Need best_tail.pt and best_fsr.pt for auto-detection, cell2=None")
    elif cell_of_interest2_input.lower() != "none":
        cell_of_interest2 = tuple(int(x) for x in cell_of_interest2_input.split(","))
        print(f"\n--- Using manual cell2: {cell_of_interest2} ---")
    
    # 第二次遍历：如果有cell2，重新统计并打印
    if cell_of_interest2 is not None:
        print(f"\n--- Re-computing stats with cell2={cell_of_interest2} ---")
        for ckpt_name, ckpt_path in checkpoints.items():
            checkpoint = torch.load(ckpt_path, map_location=args.device, weights_only=False)
            agent, _ = load_agent(ckpt_path, obs_dim, act_dim, args.device, checkpoint)
            ...
            stats = compute_visitmap_stats(
                env, agent, args.seed, args.episodes, deterministic,
                args.device, max_steps_cfg, grid_size, cell_of_interest, cell_of_interest2
            )
            
            all_stats[ckpt_name] = stats
            
            print(f"  {ckpt_name}: Hit@{cell_of_interest}={stats['hit_rate_cell']:.2%} | "
                  f"Hit@{cell_of_interest2}={stats.get('hit_rate_cell2', 0):.2%} | "
                  f"Success={stats['success_rate']:.2%} | AvgLen={stats['avg_len_success']:.1f}")
    ...
    if "best_tail.pt" in all_stats and "best_fsr.pt" in all_stats:
        diff_save_path = os.path.join(out_dir, "visitmap_diff_tail_vs_fsr.png")
        plot_visitmap_diff(
    ...
    if cell_of_interest2:
        print(f"   Cell1 (manual): {cell_of_interest}")
        print(f"   Cell2 (auto/manual): {cell_of_interest2}")
    print(f"{'='*70}\n")
...
def run_timeline_analysis(args, deterministic: bool, cell_of_interest: Tuple[int, int], cell_of_interest2_input: str):
    """
    运行 timeline 分析模式：绘制 hit rate 随训练迭代变化的曲线。
    
    Args:
        cell_of_interest2_input: "auto" or "r,c" format
    """
    print(f"\n{'='*70}")
    print(f"TIMELINE ANALYSIS MODE")
    print(f"Run Directory: {args.run_dir}")
    print(f"Timeline Glob: {args.timeline_glob}")
    print(f"Timeline Stride: {args.timeline_stride}")
    print(f"Episodes per checkpoint: {args.timeline_episodes}")
    ...
    print(f"Config: grid_size={grid_size}, max_steps={max_steps_cfg}\n")
    
    # 查找 checkpoint 文件
    ckpt_pattern = os.path.join(args.run_dir, args.timeline_glob)
    ckpt_files = glob.glob(ckpt_pattern)
    
    if not ckpt_files:
        print(f"❌ No checkpoints found matching pattern: {ckpt_pattern}")
        print(f"   Falling back to standard checkpoints...")
        ckpt_files = []
        for name in ["best_fsr.pt", "best_feasible.pt", "best_tail.pt"]:
    ...
    ckpt_files = sorted(ckpt_files, key=extract_iter)
    
    # 应用 stride
    if args.timeline_stride > 1:
        ckpt_files = ckpt_files[::args.timeline_stride]
    
    print(f"Found {len(ckpt_files)} checkpoint(s) (after stride={args.timeline_stride})")
    ...
    # 确定 cell2
    cell_of_interest2 = None
    if cell_of_interest2_input.lower() != "none":
        if cell_of_interest2_input.lower() == "auto":
            # 先快速rollout best_tail和best_fsr来确定cell2
            print(f"Auto-detecting cell2 by comparing best_tail vs best_fsr...")
            temp_stats = {}
            for name in ["best_tail.pt", "best_fsr.pt"]:
                path = os.path.join(args.run_dir, name)
                if os.path.exists(path):
                    checkpoint = torch.load(path, map_location=args.device, weights_only=False)
                    agent, _ = load_agent(path, obs_dim, act_dim, args.device, checkpoint)
                    env_temp = make_env_from_config(cfg, args.seed)
                    inject_obs_stats(env_temp, checkpoint, cfg)
                    stats = compute_visitmap_stats(
                        env_temp, agent, args.seed, min(50, args.timeline_episodes), 
                        deterministic, args.device, max_steps_cfg, grid_size, cell_of_interest, None
                    )
                    temp_stats[name] = stats
            
            if len(temp_stats) >= 2:
                tail_vp = temp_stats["best_tail.pt"]["visit_prob"]
                fsr_vp = temp_stats["best_fsr.pt"]["visit_prob"]
                delta = tail_vp - fsr_vp
                ...
                if max_cells:
                    cell_of_interest2, delta_val = max_cells[0]
                    cell_of_interest2 = (int(cell_of_interest2[0]), int(cell_of_interest2[1]))
                    print(f"   ✅ Auto-selected cell2: {cell_of_interest2} (delta={delta_val:+.3f})")
                else:
                    print(f"   ⚠️  No significant change found, using cell2=None")
            else:
                print(f"   ⚠️  Insufficient checkpoints for auto-detection, using cell2=None")
        else:
            cell_of_interest2 = tuple(int(x) for x in cell_of_interest2_input.split(","))
            print(f"Using manual cell2: {cell_of_interest2}")
    ...
    for ckpt_path in ckpt_files:
        ...
        iter_num = extract_iter(ckpt_path)
        
        print(f"Processing: {ckpt_name} (iter={iter_num})...")
        
        try:
            checkpoint = torch.load(ckpt_path, map_location=args.device, weights_only=False)
            agent, _ = load_agent(ckpt_path, obs_dim, act_dim, args.device, checkpoint)
            
            env_eval = make_env_from_config(cfg, args.seed)
            inject_obs_stats(env_eval, checkpoint, cfg)
            
            stats = compute_visitmap_stats(
                env_eval, agent, args.seed, args.timeline_episodes, deterministic,
                args.device, max_steps_cfg, grid_size, cell_of_interest, cell_of_interest2
            )
            ...
            timeline_data.append({
                "iter": iter_num,
                "ckpt_name": ckpt_name,
                "hit_rate_cell": stats["hit_rate_cell"],
                "hit_rate_cell2": stats.get("hit_rate_cell2", 0.0),
                "success_rate": stats["success_rate"],
                "avg_len_success": stats["avg_len_success"],
            })
            
            print(f"   Hit@{cell_of_interest}={stats['hit_rate_cell']:.2%}, "
                  f"Success={stats['success_rate']:.2%}")
        
        except Exception as e:
            print(f"   ⚠️  Failed to process {ckpt_name}: {e}")
            continue
    
    if not timeline_data:
        print(f"❌ No valid timeline data collected. Exiting.")
        return
    
    # 保存 CSV
    csv_path = os.path.join(out_dir, "timeline_hit_lambda.csv")
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["iter", "ckpt_name", "hit_rate_cell", "hit_rate_cell2", 
                                                "success_rate", "avg_len_success"])
        writer.writeheader()
        writer.writerows(timeline_data)
    print(f"\n✅ Timeline data saved: {csv_path}")
    
    # 绘制 timeline 图
    fig, ax1 = plt.subplots(figsize=(12, 6))
    
    iters = [d["iter"] for d in timeline_data]
    hit1 = [d["hit_rate_cell"] for d in timeline_data]
    hit2 = [d["hit_rate_cell2"] for d in timeline_data]
    success = [d["success_rate"] for d in timeline_data]
    
    ax1.plot(iters, hit1, marker="o", label=f"Hit@{cell_of_interest}", color="blue", linewidth=2)
    if cell_of_interest2:
        ax1.plot(iters, hit2, marker="s", label=f"Hit@{cell_of_interest2}", color="green", linewidth=2)
    ax1.plot(iters, success, marker="^", label="Success Rate", color="red", linewidth=2, linestyle="--")
    
    ax1.set_xlabel("Training Iteration", fontsize=12)
    ax1.set_ylabel("Rate", fontsize=12)
    ax1.set_title(f"Hit Rate Timeline: {os.path.basename(args.run_dir)}", fontsize=14, fontweight="bold")
    ax1.legend(loc="upper left", fontsize=10)
    ax1.grid(True, alpha=0.3)
    
    plt.tight_layout()
    png_path = os.path.join(out_dir, "timeline_hit_lambda.png")
    fig.savefig(png_path, dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"✅ Timeline plot saved: {png_path}")
...
def main():
    ...
    parser.add_argument("--mode", type=str, default="routes", choices=["routes", "visitmap", "timeline"],
                        help="Visualization mode: routes (trajectory) | visitmap (visit frequency) | timeline (hit rate over training)")
    parser.add_argument("--cell_of_interest", type=str, default="2,2",
                        help="Cell to track for hit rate analysis (format: r,c)")
    parser.add_argument("--cell_of_interest2", type=str, default="auto",
                        help="Second cell to track: 'auto' (auto-detect max change) | 'r,c' (manual) | 'none' (disable)")
    parser.add_argument("--topk_cells", type=int, default=1,
                        help="Number of top-k cells to report when auto-detecting cell2")
    
    # Timeline 模式专用参数
    parser.add_argument("--timeline_glob", type=str, default="checkpoint_iter_*.pt",
                        help="Glob pattern for timeline checkpoints")
    parser.add_argument("--timeline_stride", type=int, default=10,
                        help="Stride for sampling timeline checkpoints")
    parser.add_argument("--timeline_episodes", type=int, default=200,
                        help="Number of episodes per checkpoint in timeline mode")
    ...
    if args.mode == "timeline":
        run_timeline_analysis(args, deterministic, cell_of_interest, args.cell_of_interest2)
    elif args.mode == "visitmap":
        run_visitmap_analysis(args, deterministic, cell_of_interest, args.cell_of_interest2)
    else:

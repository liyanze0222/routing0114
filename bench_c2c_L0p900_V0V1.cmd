...
REM ========== Common args (match your previous benchmark exactly) ==========
...
REM start: (0~1,0~1), goal: (6~7,6~7)
set "C2C_ARGS=--start_goal_mode rect --start_rect 0,1,0,1 --goal_rect 6,7,6,7"

if not exist "%OUT%" mkdir "%OUT%"

echo ============================
echo C2C Benchmark: E=%E_BUDGET% L=%L_BUDGET% (scaled)
echo Output: %OUT%
echo ============================

REM ----------------------------
REM V0: proposed (separate cost critics, dual constraints)
REM ----------------------------
%PY% train_grid_structured_lagrangian.py %COMMON_ARGS% %C2C_ARGS% --run_tag "C2C_V0_sep_dual_E%E_BUDGET%_L%L_BUDGET%_seed0" --use_lagrange True --cost_critic_mode separate --lambda_lr_energy 0.05 --lambda_lr_load 0.05

REM ----------------------------
REM V1: unconstrained (pure reward)
REM ----------------------------
%PY% train_grid_structured_lagrangian.py %COMMON_ARGS% %C2C_ARGS% --run_tag "C2C_V1_unconstrained_E%E_BUDGET%_L%L_BUDGET%_seed0" --use_lagrange False
